...
describe("未完成验收的MOBI/KF8不发布生产入口", () => {
  ...
  afterEach(async () => { state.db?.close(); state.db = undefined; vi.restoreAllMocks(); vi.unstubAllEnvs(); const resolved = path.resolve(directory); if (path.dirname(resolved) !== path.resolve(os.tmpdir()) || !path.basename(resolved).startsWith("judu-mobi-test-")) throw new Error("不安全测试清理"); await rm(resolved, { recursive: true, force: true }); });
  it.each(["book.mobi", "book.azw", "book.azw3"])("%s明确返回415，改后缀不算KF8验收", async fileName => {
