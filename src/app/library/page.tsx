...
export default function LibraryPage(){
 ...
 return <main className="library-shell">
  ...
  <section className="library-content"><div className="library-heading"><div><div className="library-kicker">MY LIBRARY</div><h1>我的书架</h1><p>选择一本书，继续你的深度阅读。</p></div><label className="library-import">{importing?'正在导入…':'＋ 导入书籍'}<input type="file" aria-label="导入书籍文件" disabled={loading||importing} accept=".mobi,.epub,.pdf,.fb2,.fbz,.fb2.zip,.cbz,.txt,.md" onChange={event=>{const file=event.target.files?.[0];event.target.value='';if(file)void importFile(file);}}/></label></div>
   {error&&<p role="alert">{error}</p>}
   {loading?<div className="library-empty">正在读取书架……</div>:books.length===0?<div className="library-empty"><div className="empty-symbol">＋</div><h2>还没有书籍</h2><p>导入一本 MOBI、EPUB、PDF、FB2/FBZ、CBZ 或 TXT，开始第一次句读。</p></div>:<div className="book-grid">{books.map(book=><Link className="book-card" href={'/?bookId='+encodeURIComponent(book.id)} key={book.id}><div className="book-cover"><span>句读</span><i>经典原著<br/>深度阅读</i></div><div className="book-card-info"><h2>{book.title}</h2><p>{book.author}</p><span>{book.chapters.length} 个章节</span></div></Link>)}</div>}
  </section>
 </main>;
}
