...
export function OriginalInteractionLayer(props:OriginalInteractionProps){
 ...
 return <div ref={root} className="epub-interactions original-interactions" data-reader-decoration="" aria-label="原版阅读交互层">
  ...
  {active&&anchor&&<AnnotationPopover id={id} anchor={anchor} source={source} returnFocus={restoreFocus} title={active.kind==='concept'?active.target.concept.name:active.kind==='history'?'句读历史':active.kind==='mark'?(active.target.annotation.kind==='note'?'笔记':'阅读标记'):(active.preview?.title?.trim() || '引用预览')} pinned={active.pinned} onClose={close}>
   ...
   {active.kind==='link'&&<div className="epub-reference-preview">{active.preview?<><p>{active.preview.text}</p>{active.preview.address&&<p className="epub-link-address">{active.preview.address}</p>}{active.preview.index!==undefined&&<button type="button" disabled={props.disabled} onClick={()=>{const preview=active.preview,sequence=request.current,key=active.key;if(preview)void props.onJump(preview).then(()=>{if(sequence===request.current&&activeRef.current?.key===key)close();}).catch((cause:unknown)=>{console.warn('引用跳转失败',cause);if(sequence===request.current)props.onNotice('引用跳转失败，请从原书目录重试。');});}}>跳转到原文</button>}</>:<p role="status">正在读取引用…</p>}</div>}
  </AnnotationPopover>}
 </div>;
}
