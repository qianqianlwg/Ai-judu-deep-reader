...
describe("Agent 工具 schema", () => {
  ...
  it("模型不能在保存参数中注入消息或原文锚点", () => { const value = { summary: "解释", breakdown: [], concepts: [], context: "", uncertainty: "", citations: [] }; expect(saveAnalysisSchema.safeParse(value).success).toBe(true); expect(saveAnalysisSchema.safeParse({ ...value, anchor: {}, messageId: "other" }).success).toBe(false); });
});
