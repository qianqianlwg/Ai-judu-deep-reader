...
function fixture() {
  const deps: ReadingToolDependencies = { messageId: "m", selectedText: source.text, sources: new Map(), search: vi.fn(async () => [source]), read: vi.fn(async () => [source]), save: vi.fn(async () => undefined), anchor: { paragraphId: "p", startOffset: 0, endOffset: source.text.length, selectedText: source.text } };
  return { deps, tools: createReadingTools(deps) };
}
const analysis = { summary: "解释", breakdown: [], concepts: [{ name: "认识", text: "认识活动" }], context: "", uncertainty: "", citations: [{ sourceId: source.sourceId, quote: "认识" }] };
describe("结构化句读工具", () => {
