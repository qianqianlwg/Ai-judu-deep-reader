...
function parser(html='<p id="note">真实目标</p><a href="kindle:pos:fid:0000:off:0000">注释</a>'):MobiCandidate {
 ...
 return {getMetadata:()=>({title:'书',author:['作者']}),getSpine:()=>[{id:'0'}],getSourceChapter:()=>({id:'0',encoding:65001,bytes:new TextEncoder().encode(html),spans:[{fid:0,start:0,end:new TextEncoder().encode(html).length,targetStart:0}]}),getToc:()=>[],getCoverImage:()=>'',loadChapter:()=>({html,head:'<style>p{color:red}</style>',css:[]}),resolveHref:vi.fn(()=>({id:'0',selector:'[id="note"]'}))};
}
