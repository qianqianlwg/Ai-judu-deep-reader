...
export type ReadingAnnotationsOptions = {
  ...
  setSelectionMenu: (value: { left: number; top: number } | null) => void; openConversation: (threadId: string, messageId: string | null) => void;
};
...
export function useReadingAnnotations(options: ReadingAnnotationsOptions): ReadingAnnotationsResult {
  const { bookId, editionId, sourceParagraphs, bookConcepts, selectionAnchor, setNotice, setWorkspaceView, setSelected, setSelectionAnchor, setReadingAnchor, setActiveSource, setSelectionMenu, openConversation } = options;
  ...
  const openAnnotation = useCallback((annotation: TextAnnotation): void => {
    setWorkspaceView("reader"); const paragraph = sourceParagraphs.find(item => item.id === annotation.paragraphId); const text = paragraph?.text.slice(annotation.startOffset, annotation.endOffset) ?? "";
    setSelected(text); setSelectionAnchor({ paragraphId: annotation.paragraphId, startOffset: annotation.startOffset, endOffset: annotation.endOffset, text });
    setReadingAnchor({ paragraphId: annotation.paragraphId, offset: annotation.startOffset }); setActiveSource(annotation.paragraphId); openConversation(annotation.threadId, annotation.messageId ?? null);
  }, [openConversation, setActiveSource, setReadingAnchor, setSelected, setSelectionAnchor, setWorkspaceView, sourceParagraphs]);
  ...
  const saveManualMark = useCallback(async (kind: ReadingMark["kind"], note = "", color: SelectionMarkColor = "yellow"): Promise<void> => {
    ...
    try {
      const response = await fetch("/api/reading-marks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ editionId, kind, color, note, anchors: [{ paragraphId: selectionAnchor.paragraphId, startOffset: selectionAnchor.startOffset, endOffset: selectionAnchor.endOffset, selectedText: selectionAnchor.text, textHash: hashText(selectionAnchor.text) }] }) });
