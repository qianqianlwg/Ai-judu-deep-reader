...
- 遇到疑似 Codex、内置浏览器、自动化工具或编译验证环境导致的异常，应暂停相关开发与验收，说明证据并请用户确认环境。
- 未经用户确认，不得为通过开发工具验证而更改产品渲染方案、依赖行为或安全策略（例如增加替代文档运输、修改上游 shadow root 以绕过工具限制）。
- 环境确认前不扩大兼容补丁，也不把验证环境异常直接认定为产品缺陷；保留与该异常无关的安全、数据和业务修复。

## 用户验收使用主项目

- 隔离端口、独立数据目录或子项目仅用于 AI 自测，不再交给用户验收。
- 交付用户验证前，先完成隔离自测，再启动或重启主项目 `D:\project\Ai-judu-deep-reader`，使用主端口 `3000`、默认构建目录 `.next` 和正式数据目录 `data`。
- 不把 `.next-verify-*` 构建或临时测试书库当作主项目；保留用户已有书籍、设置和标注，不清空正式数据。
- 确认主项目首页和书库 API 正常后，只提供主项目地址 `http://localhost:3000/`。反馈主项目重启结果与临时端口释放状态。
